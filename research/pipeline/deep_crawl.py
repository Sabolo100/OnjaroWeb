...
# Company enrichment queries — for skeleton companies (confidence <= 0.4)
_COMPANY_ENRICH_QUERIES = [
    '"{company}" facility management property management cég bemutatkozás',
    '"{company}" szolgáltatások irodaház üzemeltetés kezelés',
]

# Confidence threshold below which a company is considered "skeleton"
SKELETON_CONFIDENCE_THRESHOLD = 0.45
...
def get_companies_to_crawl(max_count: int = MAX_COMPANIES_PER_RUN) -> List[dict]:
    ...
    try:
        ...
        resp = client.table("companies").select(
            "id,name,website,deep_crawled_at,confidence"
        ).execute()
        ...
        # Sort: skeleton companies first (low confidence), then never-crawled, then oldest
        def _sort_key(c):
            conf = c.get("confidence") or 0.5
            is_skeleton = 0 if conf <= SKELETON_CONFIDENCE_THRESHOLD else 1
            crawled = c.get("deep_crawled_at") or ""
            return (is_skeleton, crawled)
...
def generate_deep_crawl_items(companies: List[dict]) -> List[dict]:
    ...
    for company in companies:
        ...
        confidence = company.get("confidence") or 0.5
        is_skeleton = confidence <= SKELETON_CONFIDENCE_THRESHOLD

        # Company enrichment item — only for skeleton companies
        if is_skeleton:
            items.append({
                "id": f"deep_crawl_enrich_{company_id[:12]}",
                "name": f"Deep crawl: {name} — cégadatok feltöltése",
                "enabled": True,
                "priority": 5,  # Higher priority than people/buildings
                "target_table": "companies",
                "search_type": "topic_search",
                "schema_name": "company",
                "schedule": "manual",
                "max_results_per_run": 3,
                "topics": [q.format(company=name) for q in _COMPANY_ENRICH_QUERIES],
                "language": "hu",
                "content_types": ["company"],
                "min_confidence": 0.5,
                "auto_approve_above": 0.7,
                "manual_review_below": 0.3,
                "_deep_crawl_company_id": company_id,
                "_deep_crawl_company_name": name,
                "_deep_crawl_enrich": True,
            })
